...
public class SyncAdJob extends Job {
    private static final String LOG_TAG = SyncAdJob.class.getSimpleName();
    ...
    public static void scheduleJob() {
        new JobRequest.Builder(SyncAdJob.TAG)
                ...
                if (response.isSuccessful()) {
                    ...
                    if (response.body() != null) {
                        ...
                        if (adRequested != null) {
                            ...
                            extraAd.setAdPath(adRequested.getAdPath());
                            extraAd.setAdId(adRequested.getAdId());
                            extraAd.setButtonName(adRequested.getButtonName());
                            extraAd.setButtonLink(adRequested.getButtonLink());
                            extraAd.setButtonDestination(adRequested.getButtonDestination());
                            AppDatabase.getInstance(getContext()).getExtraAdDao().deleteTable();
                            AppDatabase.getInstance(getContext()).getExtraAdDao().insertExtraAd(extraAd);
    ...
    private void syncExtraAdJob() {
        JobManager.create(getContext()).addJobCreator(new ExtraAdJobCreator());
        int jobId = new JobRequest.Builder(SyncExtraAdJob.TAG)
