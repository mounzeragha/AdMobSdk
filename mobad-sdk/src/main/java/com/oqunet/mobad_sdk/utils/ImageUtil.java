...
public class ImageUtil {
	...
	public static void displayRoundImage(ImageView view, String path, ImageLoadingListener listener) {
		ImageLoader loader = ImageLoader.getInstance();
		try {
			loader.displayImage(path, view, DEFAULT_DISPLAY_ROUNDED_IMAGE_OPTIONS, listener);
		} catch (OutOfMemoryError e) {
			e.printStackTrace();
			loader.clearMemoryCache();
		}
	...
	private static final DisplayImageOptions.Builder DEFAULT_DISPLAY_ROUNDED_IMAGE_OPTIONS_BUIDLER = new DisplayImageOptions.Builder()
			.imageScaleType(ImageScaleType.IN_SAMPLE_POWER_OF_2)
			.displayer(new FadeInBitmapDisplayer(300, true, false, false))
			.showImageForEmptyUri(R.drawable.default_image_rounded)
			.showImageOnLoading(R.drawable.default_image_rounded)
			.showImageOnFail(R.drawable.default_image_rounded).cacheOnDisk(true)
			.cacheInMemory(true).bitmapConfig(Config.ARGB_8888);
